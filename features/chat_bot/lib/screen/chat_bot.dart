...
class _ChatBotScreenState extends State<ChatBotScreen> {
  ...
  void sendMessage(String message) {
    final request = CompleteReq(prompt: message, model: kTranslateModelV3,max_tokens: 2000);

    subscription = openAI.onCompleteStream(request: request)
        .asBroadcastStream()
        .listen((it) {
      setState(() {
        messages.add(MessageModel(true,"${it?.choices.last.text}"));
      });
    });
  }
            ...
            decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(
                      color: Colors.grey.withOpacity(.12),
                      offset:
                      const Offset(kDefault / 1.2, kDefault / 2),
                      blurRadius: kDefault * 2)
                ]),
